...
import { GoalTaskNode } from "./nodes/goal-task-node";
...
const nodeTemplates = [
  {
    type: "milestone" as JourneyNodeType,
    label: "",
    description: "",
...
export function WorkflowCanvas() {
  ...
  // Track which journey we've fitted view for to prevent re-running
  const fittedViewForJourneyRef = useRef<string | null | undefined>(undefined);
  ...
  }, [currentJourneyId, hasRealNodes, fitView]);
  ...
  const handleConnectionToNewNode = useCallback(
    (event: MouseEvent | TouchEvent, clientX: number, clientY: number) => {
      ...
      // Default to goal type when adding from sidebar
      const nodeTemplate =
        nodeTemplates.find((t) => t.type === "goal") || nodeTemplates[0];
      if (!nodeTemplate) {
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-auto">
        <WorkflowToolbar workflowId={currentJourneyId ?? undefined} />
      </div>
